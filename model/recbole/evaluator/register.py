...
"""
model.recbole.evaluator.register
################################################
"""
import inspect
import sys
...
def cluster_info(module_name):
    ...
    Note:
        For ``metric_type``: in current model.recbole, all the "grouped-score" metrics are ranking-based and all the
        "non-grouped-score" metrics are value-based. To keep with our paper, we adopted the more formal terms:
        ``RANKING`` and ``VALUE``.

    Args:
        module_name (str): the name of module ``model.recbole.evaluator.metrics``.
...
metric_module_name = 'model.recbole.evaluator.metrics'
