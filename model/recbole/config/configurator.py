...
"""
model.recbole.config.configurator
################################
"""
...
from model.recbole.evaluator import metric_types, smaller_metrics
from model.recbole.utils import get_model, Enum, EvaluatorType, ModelType, InputType, \
...
class Config(object):
    ...
    Configurator module will first load the default parameters from the fixed properties in model.recbole and then
    ...
    def _load_cmd_line(self):
        ...
        if len(unrecognized_args) > 0:
            logger = getLogger()
            logger.warning('command line args [{}] will not be used in model.recbole'.format(' '.join(unrecognized_args)))
