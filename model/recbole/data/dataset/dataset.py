...
from model.recbole.data.interaction import Interaction
from model.recbole.utils import FeatureSource, FeatureType, get_local_time, set_color, ensure_dir
from model.recbole.utils.url import decide_download, download_url, extract_zip, makedirs, rename_atomic_files
...
class Dataset:
    ...
    imputation. Features are stored as :class:`pandas.DataFrame` inside :class:`~model.recbole.data.dataset.dataset.Dataset`.
    General and Context-aware Models can use this class.

    By calling method :meth:`~model.recbole.data.dataset.dataset.Dataset.build`, it will processing dataset into
    DataLoaders, according to :class:`~model.recbole.config.eval_setting.EvalSetting`.
    ...
    Attributes:
        ...
        field2type (dict): Dict mapping feature name (str) to its type (:class:`~model.recbole.utils.enum_type.FeatureType`).

        field2source (dict): Dict mapping feature name (str) to its source
            (:class:`~model.recbole.utils.enum_type.FeatureSource`).
    ...
    def _data_filtering(self):
        ...
        Note:
            ...
            thus :meth:`~model.recbole.data.dataset.dataset.Dataset._reset_index` will reset the index of feats.
        """
    ...
    def _load_user_or_item_feat(self, token, dataset_path, source, field_name):
        ...
        Note:
            ``user_id`` and ``item_id`` has source :obj:`~model.recbole.utils.enum_type.FeatureSource.USER_ID` and
            :obj:`~model.recbole.utils.enum_type.FeatureSource.ITEM_ID`
        """
    ...
    def _fill_nan(self):
        ...
        For fields with type :obj:`~model.recbole.utils.enum_type.FeatureType.TOKEN`, missing value will be filled by
        ``[PAD]``, which indexed as 0.

        For fields with type :obj:`~model.recbole.utils.enum_type.FeatureType.FLOAT`, missing value will be filled by
    ...
    def float_like_fields(self):
        """Get fields of type :obj:`~model.recbole.utils.enum_type.FeatureType.FLOAT` and
        :obj:`~model.recbole.utils.enum_type.FeatureType.FLOAT_SEQ`.
    ...
    def token_like_fields(self):
        """Get fields of type :obj:`~model.recbole.utils.enum_type.FeatureType.TOKEN` and
        :obj:`~model.recbole.utils.enum_type.FeatureType.TOKEN_SEQ`.
    ...
    @property
    def seq_fields(self):
        """Get fields of type :obj:`~model.recbole.utils.enum_type.FeatureType.TOKEN_SEQ` and
        :obj:`~model.recbole.utils.enum_type.FeatureType.FLOAT_SEQ`.
    ...
    def non_seq_fields(self):
        """Get fields of type :obj:`~model.recbole.utils.enum_type.FeatureType.TOKEN` and
        :obj:`~model.recbole.utils.enum_type.FeatureType.FLOAT`.
    ...
    def build(self):
        ...
        See :class:`~model.recbole.config.eval_setting.EvalSetting` for details.
    ...
    def _dataframe_to_interaction(self, data):
        """Convert :class:`pandas.DataFrame` to :class:`~model.recbole.data.interaction.Interaction`.

        Args:
            data (pandas.DataFrame): data to be converted.

        Returns:
            :class:`~model.recbole.data.interaction.Interaction`: Converted data.
        """
        new_data = {}
        for k in data:
            value = data[k].values
