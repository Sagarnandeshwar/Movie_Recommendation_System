...
from model.recbole.utils import FeatureSource, FeatureType, set_color
from model.recbole.utils.url import decide_download, download_url, extract_zip


class KnowledgeBasedDataset(Dataset):
    """:class:`KnowledgeBasedDataset` is based on :class:`~model.recbole.data.dataset.dataset.Dataset`,
    ...
    def _download(self):
        ...
        if decide_download(url):
            ...
            self.logger.info(
                ...
                f'You can run model.recbole after the conversion, see you soon.'
            )
            exit(0)
        else:
